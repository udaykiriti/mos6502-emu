...
#define DOCTEST_SUPPRESS_PRIVATE_WARNINGS_PUSH                                 \
  ...
  DOCTEST_MSVC_SUPPRESS_WARNING(                                               \
      4530) /* exception handler, but unwind semantics not enabled */          \
  DOCTEST_MSVC_SUPPRESS_WARNING(                                               \
      4577) /* 'noexcept' with no exception handling mode specified */         \
  DOCTEST_MSVC_SUPPRESS_WARNING(                                               \
      4774) /* format string in argument is not a string literal */            \
  ...
  DOCTEST_MSVC_SUPPRESS_WARNING(                                               \
      5245) /* unreferenced function with internal linkage removed */
...
// for clang - always include <version> or <ciso646> (which drags some std
// stuff) because we want to check if we are using libc++ with the
// _LIBCPP_VERSION macro in which case we don't want to forward declare stuff
// from std - for reference: https://github.com/doctest/doctest/issues/126
...
template <typename L> struct Expression_lhs {
  ...
  DOCTEST_NOINLINE operator Result() {
    ...
    bool res = static_cast<bool>(
        lhs); // NOLINT(bugprone-non-zero-enum-to-bool-conversion)
    DOCTEST_MSVC_SUPPRESS_WARNING_POP
    if (m_at & assertType::is_false) {
      res = !res;
    }
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(translatorName, signature)  \
  ...
  DOCTEST_GLOBAL_NO_WARNINGS(                                                  \
      DOCTEST_ANONYMOUS(                                                       \
          DOCTEST_ANON_TRANSLATOR_VAR_), /* NOLINT(cert-err58-cpp) */          \
...
#define DOCTEST_REGISTER_REPORTER(name, priority, reporter)                    \
  DOCTEST_GLOBAL_NO_WARNINGS(                                                  \
      DOCTEST_ANONYMOUS(                                                       \
          DOCTEST_ANON_REPORTER_VAR_), /* NOLINT(cert-err58-cpp) */            \
...
private:
  // decisionPath is the selected traversal prefix; discoveredDecisionPath is
  // rebuilt on each rerun to describe the branches encountered at each depth.
...
const char *assertString(assertType::Enum at) {
  DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(
      4061) // enum 'x' in switch of enum 'y' is not explicitly handled
...
// The following code has been taken verbatim from
// Catch2/include/internal/catch_xmlwriter.h This is done so cherry-picking bug
// fixes is trivial - even the style/formatting is untouched.
...
int Context::run() {
  ...
  for (auto &curr : testArray) {
    ...
    // execute the test if it passes all the filtering
    {
      ...
      do { // NOLINT(cppcoreguidelines-avoid-do-while)
        // Reset per-run traversal data while keeping the current decision path
        // prefix.
        ...
        try {
#endif // DOCTEST_CONFIG_NO_EXCEPTIONS
       // MSVC 2015 diagnoses fatalConditionHandler as unused (because reset()
       // is a static method)
          DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(
...
FatalConditionHandler::FatalConditionHandler() {
  ...
  std::set_terminate([]() DOCTEST_NOEXCEPT {
    ...
    if (isDebuggerActive() && !g_cs->no_breaks)
      DOCTEST_BREAK_INTO_DEBUGGER();
    std::exit(EXIT_FAILURE); // explicitly exit - otherwise the SIGABRT handler
...
String::size_type String::size() const {
  if (isOnStack())
    return last - (static_cast<size_type>(buf[last]) &
...
TestCase::TestCase(
    const TestCase &other) noexcept // NOLINT(bugprone-copy-constructor-init)
    : TestCaseData() {
  *this = other;
}
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(26434) // hides a non-virtual function
TestCase &
TestCase::operator=(const TestCase &other) noexcept { // NOLINT(cert-oop54-cpp)
...
// The following code has been taken verbatim from
// Catch2/include/internal/catch_xmlwriter.cpp This is done so cherry-picking
// bug fixes is trivial - even the style/formatting is untouched.
/* clang-format off */ /* NOLINTBEGIN */
...
#endif // defined(DOCTEST_CONFIG_IMPLEMENT) &&
       // !defined(DOCTEST_LIBRARY_IMPLEMENTATION)